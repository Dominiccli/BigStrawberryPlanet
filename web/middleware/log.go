...
func LoggerToFile(logger *logrus.Logger) gin.HandlerFunc {
	...
	LogFile := wdPath + conf.LOGFILESUBPATH + conf.LOGFILENAME

	// 写入文件
	logFileFd, err := os.OpenFile(LogFile, os.O_CREATE | os.O_APPEND | os.O_RDWR, 0666)
	if err != nil {
		fmt.Println("err", err)
	}
	// 实例化
